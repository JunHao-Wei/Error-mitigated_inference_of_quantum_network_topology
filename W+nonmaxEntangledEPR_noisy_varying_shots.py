...
prep_nodes = [
    qnet.PrepareNode(1, [0,1,2], qnet.W_state, 0),
    qnet.PrepareNode(1, [3,4], lambda settings, wires: None, 0),
]
# ~ qnet.nonmax_entangled_state(settings=[math.pi/4], wires=[3,4])
def processing_ansatz(settings, wires):
	qml.RY(math.pi/4, wires=wires[3])
	qml.CNOT(wires=wires[3:5])
...
def UNCERTAINTY_local(m, ansatz):
	...
	def Uncertainty_local(*settings):
		...
		uncertainty_local = qnet.shannon_entropy(Probs_basis1) + qnet.shannon_entropy(Probs_basis2)
...
W1 = - 2 * np.log2(2/3) / 3 - np.log2(1/3) / 3 
W2 = 0.349976
W3 = 1.29711
W4 = 0.600877
matrix_ideal_uncer = [[W1+1,W3,W3,W1+1,W1+1], [W3,W1+1,W3,W1+1,W1+1], [W3,W3,W1+1,W1+1,W1+1], [W4+1,W4+1,W4+1,W4+1,W4], [W4+1,W4+1,W4+1,W4,W4+1]]
matrix_ideal_mutualinfo = [[W1,W2,W2,0,0], [W2,W1,W2,0,0], [W2,W2,W1,0,0], [0,0,0,W4,W4], [0,0,0,W4,W4]]
matrix_ideal_covar = [[1,2/3,2/3,0,0], [2/3,1,2/3,0,0], [2/3,2/3,1,0,0], [0,0,0,1,math.sqrt(2)/2], [0,0,0,math.sqrt(2)/2,1]]
...
Optimization_wrt_steps(20, 70, 70, 0.1, 0.2, 0.1, 0.3, 0.2)
# ~ Optimization_wrt_steps(trials, steps_uncer, steps, stepsizeUncerLocal, stepsizese, stepsizeUncer, stepsizeMutualInfo, stepsizeCovar)
