...
def Optimization_EPR_VD(trials, steps, gamma, stepsizeUncern):
	...
	def prepare_ansatz(settings,wires):
		qml.QubitDensityMatrix(rho, wires=wires[0:2])
		qml.QubitDensityMatrix(rho, wires=wires[2:4])
		qml.QubitDensityMatrix(rho, wires=wires[4:6])
		qml.QubitDensityMatrix(rho, wires=wires[6:8])
	ghz_prep_nodes_VD = [
		qnet.PrepareNode(
			wires=[0,1,2,3,4,5,6,7],
			ansatz_fn=prepare_ansatz,
			num_settings=0
		)
	]
...
ghz_prep_node = [
    qnet.PrepareNode(1, [0,1,4], qnet.ghz_state, 0)
]
...
Uncern_depolarizing_withoutVD = - (((4*a/3)**2 - 2*(4*a/3) +2)/2) * log2(((4*a/3)**2 - 2*(4*a/3) +2)/4) - ((-(4*a/3)**2 + 2*(4*a/3))/2) * log2((-(4*a/3)**2 + 2*(4*a/3))/4)
Uncern_depolarizing_withVD = -(((4*a/3)**2 * ((4*a/3)-2)**2 )/(2 * (4*a/3)**2 * ((4*a/3)-2)**2 + 4 * ((4*a/3)-1)**2)) * log2(((4*a/3)**2 * ((4*a/3)-2)**2 )/(4 * (4*a/3)**2 * ((4*a/3)-2)**2 + 8 * ((4*a/3)-1)**2)) - (((4*a/3)**2 * ((4*a/3)-2)**2 + 4*((4*a/3)-1)**2)/(2 * (4*a/3)**2 * ((4*a/3)-2)**2 + 4 * ((4*a/3)-1)**2)) * log2(((4*a/3)**2 * ((4*a/3)-2)**2 + 4*((4*a/3)-1)**2)/(4 * (4*a/3)**2 * ((4*a/3)-2)**2 + 8 * ((4*a/3)-1)**2))
...
for gamma in np.arange(0,0.76,0.05):
	Optimization_EPR_VD(20, 20, gamma, 0.2+gamma*2)
	Optimization_EPR(20, 20, gamma, gamma, 0.3+gamma)


fig1 = plt.figure('Optimization for Noisy GHZ with error band', figsize = (8,7)).add_subplot(111)
...
fig1.plot(np.linspace(0,1,99), np.insert(Uncern_depolarizing_withoutVD_values,0,1), color='deepskyblue', label = 'Theoretical value without VD')
fig1.plot(np.linspace(0,1,99), np.insert(Uncern_depolarizing_withVD_values,0,1), color='orange', label = 'Theoretical value with VD')
fig1.set_title('Optimization for Noisy EPR with and without VD', size=24)
...
fig2 = plt.figure('Optimization for Noisy GHZ without error band', figsize = (8,7)).add_subplot(111)
...
# ~ fig2.scatter(np.arange(0,0.76,0.05), np.array(opt_score_condQ1_EPR_min), s=24, c='#1F77B4', label='Minimum uncertainty without VD', marker='v')
# ~ fig2.scatter(np.arange(0,0.76,0.05), np.array(opt_score_condQ1_EPR_VD_min), s=24, c='#FF7F0E', label='Minimum uncertainty with VD', marker='v')
fig2.plot(np.linspace(0,1,99), np.insert(Uncern_depolarizing_withoutVD_values,0,1), color='deepskyblue', label = 'Theoretical value without VD')
fig2.plot(np.linspace(0,1,99), np.insert(Uncern_depolarizing_withVD_values,0,1), color='orange', label = 'Theoretical value with VD')
fig2.set_title('Optimization for Noisy EPR with and without VD', size=24)
