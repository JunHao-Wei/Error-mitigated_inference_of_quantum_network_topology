...
def processing_ansatz(settings, wires):
	qml.RY(math.pi/4, wires=wires[0])
...
def processing_ansatz_alt(settings, wires):
	qml.RY(math.pi/4, wires=wires[0])
	qml.CNOT(wires=wires[0:2])
...
proc_nodes_alt = [
	qnet.ProcessingNode(
		wires=[0,1],
		ansatz_fn=processing_ansatz_alt
	)
]
...
def Optimization_EPR(trials, shots, steps):
	...
	# ~ fig1.plot(range(0,steps+1), np.array(np.multiply(scores_aver_condQ1_EPR, 0.5)) - W/2, label="Uncertainty", linewidth=2)
	# ~ fig1.plot(range(0,steps+1), W - np.array(scores_aver_mutual_info_EPR), label="Mutual information", linewidth=2)
	# ~ fig1.plot(range(0,steps+1), C/2 - np.array(np.multiply(scores_aver_covar_EPR, 0.5)), label="Covariance", linewidth=2)#Halved, since covariance ranges from -1 to 1.
	# ~ fig1.fill_between(range(0,steps+1), np.array(np.multiply(scores_aver_condQ1_EPR, 0.5)) - W/2 + np.array(np.multiply(scores_condQ1_EPR_standarderror, 0.5)), np.array(np.multiply(scores_aver_condQ1_EPR, 0.5)) - W/2 - np.array(np.multiply(scores_condQ1_EPR_standarderror, 0.5)), alpha=.5, linewidth=0)
	# ~ fig1.fill_between(range(0,steps+1), W - (np.array(scores_aver_mutual_info_EPR) + np.array(scores_mutual_info_EPR_standarderror)), W - (np.array(scores_aver_mutual_info_EPR) - np.array(scores_mutual_info_EPR_standarderror)), alpha=.5, linewidth=0)
	# ~ fig1.fill_between(range(0,steps+1), C/2 - (np.array(np.multiply(scores_aver_covar_EPR, 0.5)) + np.array(np.multiply(scores_covar_EPR_standarderror, 0.5))), C/2 - (np.array(np.multiply(scores_aver_covar_EPR, 0.5)) - np.array(np.multiply(scores_covar_EPR_standarderror, 0.5))), alpha=.5, linewidth=0)
	...
	fig2 = plt.figure('Scatter points of mutual information', figsize=(8, 5)).add_subplot(111)
	for t in range(steps+1):
		plt.scatter([t] * trials, np.array(scores_mutual_info_EPR).transpose()[t], alpha=0.5, s=6, color='#ea841e')
	fig2.axhline(W, color='black', linestyle='--', linewidth=1.5)
	fig2.set_title('Scatter points of mutual information', size=24)
	fig2.set_ylabel('Value of mutual information', size=16)
	fig2.set_xlabel('Gradient Descent Iteration', size=16)
Optimization_EPR(50, 10000, 50)
